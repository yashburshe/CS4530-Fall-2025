...
- `changeCommunityMembership`: Toggles user membership in a community
    - The changeCommunityMembership function adds or removes a user from a community using toggle logic. Send a POST request via api.post2()  with { communityId, username } in the request body. Confirm status 200; if not, throw "Error while changing community membership". Return res.data containing the updated community.
